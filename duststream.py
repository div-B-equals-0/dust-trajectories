...
class DustStream(object):
    ...
    def __init__(
        ...
        self.drag_constant = (4.0 / self.Qp) * (
            self.cs * self.taustar * self.kappa_d / (self.vinf * self.kappa)
        ) ** 2
        ...
        # P_rad/P_gas at R**
        self.Upstarstar = (self.vinf / self.cs) ** 2 * (
            self.Rstar / self.Rstarstar
        ) ** 2
...
def total_accel(x, w, s):
    ...
    `DustStream` object.
    """
    return 0.5 * (x ** (-2) - s.drag_constant * ds79.Fdrag(w, s.T, phi(x, s), s.n))
...
def vector_accel_3d_frozen(x, y, z, vx, vy, vz, s, vxgas=-1.0, vygas=0.0, vzgas=0.0):
    """
