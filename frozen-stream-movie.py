...
fileroot = sys.argv[0].replace(".py", "-" + stream_id)
sns.set_color_codes()
fig, ax = plt.subplots(figsize=(5, 5))


# Artist to use for animation of streamlines
point_artists = [ax.plot([], [], '.', ms=4.0, color='r', zorder=100)[0] for _ in xs]
nB = 60
line_artists = [ax.plot([], [], lw=2, alpha=0.5, color='c')[0] for _ in range(nB)]
xxBs = np.linspace(4*xmin, 4*xmax, nB)
...
def animate_points_and_lines(itime):
    ...
    for xx, artist in zip(xxBs, line_artists):
        yy1, yy2 = 6*ymin, 6*ymax
        x1 = -xx*sthB + yy1*cthB  + (Xstart - tstop*itime/nt)*traj.Rsd
        x2 = -xx*sthB + yy2*cthB  + (Xstart - tstop*itime/nt)*traj.Rsd
        y1 = xx*cthB + yy1*sthB
        y2 = xx*cthB + yy2*sthB
...
for x, y in zip(xs, ys):
    ax.plot(x, y, '-', color='w', lw=0.7, alpha=0.6)
    ...
    #             '.', ms=4.0, color=color, zorder=20-x1, alpha=0.2)
